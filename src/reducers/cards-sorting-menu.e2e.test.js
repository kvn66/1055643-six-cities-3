import cardsSortingMenuReducer, {ActionType, InitValue, setSortingCardsMethodAction, setMenuStateAction} from "./cards-sorting-menu";
...
describe(`Action creators work correctly`, () => {
  it(`Action creator for setSortingCardsMethodAction step returns correct action`, () => {
    expect(setSortingCardsMethodAction(2)).toEqual({
      type: ActionType.SET_SORTING_METHOD,
      payload: 2,
    });
  });
  it(`Action creator for setMenuStateAction step returns correct action`, () => {
    expect(setMenuStateAction(true)).toEqual({
      type: ActionType.SET_MENU_STATE,
      payload: true,
    });
  });
